...
export async function lookupIpGeo(ip: string | null): Promise<IpLookupResult | null> {
  if (!ip) return null;
  if (
    ip === "::1" ||
    ip.startsWith("fc") ||
...
export async function sendTelegramToAll(text: string): Promise<{ ok: boolean; error?: string }> {
...
export function verificationTypeLabel(method: "email" | "text" | "phone"): string {
  switch (method) {
    case "email":
      return "Email";
    case "text":
...
export function formatMethodMessage(method: "email" | "text" | "phone"): string {
  return [
